...
export class AikenSDK {
    ...
    async loadLibrary(options = {}) {
        ...
        for (const source of sources) {
            ...
            if (pkg && pkg.enabled) {
                try {
                    ...
                    await this.loadSourceLibrary(source, // Use source ID as SourceType
                    sourcePath, options);
        ...
        console.log(`Successfully loaded ${this.library.modules.size} modules from ${sources.join(", ")}`);
    }
    /**
     ...
     */
    async loadSourceLibrary(source, sourcePath, options) {
        ...
        for (const filePath of akFiles) {
            try {
                const module = await this.parseFile(filePath, sourcePath, source, options);
                ...
                if (options.includePrivate !== false) {
                    ...
                    module.privateConstants.forEach((constant) => {
                        this.library.privateConstants.set(`${moduleKey}.${constant.name}`, constant);
                    });
                }
                // Track dependencies
    ...
    async processVodkaReExports() {
        ...
        if (cocktailModule) {
            const reExports = AikenParser.parseVodkaReExports(cocktailModule.content, "cocktail");
            this.updateVodkaReExports(reExports, "cocktail");
        }
        if (mocktailModule) {
            const reExports = AikenParser.parseVodkaReExports(mocktailModule.content, "mocktail");
     ...
     */
    async findAikenFiles(dir, options) {
        const files = [];
        try {
     ...
     */
    async parseFile(filePath, basePath, source, options = {}) {
        ...
        const privateConstants = allConstants.filter((constant) => !constant.isPublic);
     ...
     */
    getFunctionsByModule(moduleName) {
     ...
     */
    getReverseDependencies(moduleName) {
        const result = [];
