export class OffchainParser {
     ...
     */
    static parseExports(content, source = "offchain") {
        const exports = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // export * from './module'
            // Named exports
            const namedExportMatch = line.match(/^export\s+(function|const|class|interface|type|enum|namespace)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(?/);
            if (namedExportMatch) {
            ...
            // Default exports
            const defaultExportMatch = line.match(/^export\s+default\s+(function|const|class|interface|type|enum)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(?/);
            ...
            if (exportListMatch) {
                const items = exportListMatch[1]
                    ...
                    .filter(Boolean);
                items.forEach((item) => {
                    const nameMatch = item.match(/([a-zA-Z_][a-zA-Z0-9_]*)(?:\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*))?/);
                    if (nameMatch) {
     ...
     */
    static parseFunctions(content, source = "offchain") {
        const functions = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            const functionMatch = line.match(/^(?:\s*)(?:export\s+)?(?:public\s+)?(?:function\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(|const\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*\(|([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*\([^)]*\)\s*=>|([a-zA-Z_][a-zA-Z0-9_]*)\s*\([^)]*\)\s*:\s*[^{]*\{|([a-zA-Z_][a-zA-Z0-9_]*)\s*\([^)]*\)\s*[:=])/);
            if (functionMatch) {
                const [, fnName, constName, arrowName, classMethodName, methodName] = functionMatch;
                const name = fnName || constName || arrowName || classMethodName || methodName;
                if (name) {
     ...
     */
    static parseClasses(content, source = "offchain") {
        const classes = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            // export class name
            const classMatch = line.match(/^(?:export\s+)?class\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:extends\s+([a-zA-Z_][a-zA-Z0-9_]*))?\s*(?:implements\s+([^{]+))?\s*\{?/);
            if (classMatch) {
     ...
     */
    static parseTypes(content, source = "offchain") {
        const types = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            // export type name = ...
            // export interface name { ... }
            const typeMatch = line.match(/^(?:export\s+)?(type|interface)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:extends\s+([^{]+))?\s*[={\s]/);
            if (typeMatch) {
     ...
     */
    static parseConstants(content, source = "offchain") {
        const constants = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            const constMatch = line.match(/^(?:export\s+)?const\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*(?::\s*([^=]+))?\s*=\s*(.+)$/);
            if (constMatch) {
     ...
     */
    static extractFunctionBlock(lines, startLine) {
        let documentation = "";
        let implementation = "";
        let fullBlock = "";
     ...
     */
    static parseTypeScriptParameters(paramsStr) {
        if (!paramsStr.trim()) {
            return [];
        }
        ...
        return params.map((param) => {
            ...
            const paramMatch = param.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:\?)?\s*(?::\s*([^=]+))?\s*(?:=\s*(.+))?$/);
            if (paramMatch) {
     ...
     */
    static splitTypeScriptParameters(params) {
        const result = [];
        let current = "";
        let braceCount = 0;
        let bracketCount = 0;
     ...
     */
    static parseClassMethods(classBody, startLine) {
        const methods = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const methodMatch = line.match(/^(?:public\s+|private\s+|protected\s+)?(?:static\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*(?::\s*([^{]+))?\s*\{?/);
            if (methodMatch) {
     ...
     */
    static parseClassProperties(classBody, startLine) {
        const properties = [];
        const lines = classBody.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            // readonly name: type
            // static name: type
            const propertyMatch = line.match(/^(?:public\s+|private\s+|protected\s+)?(?:readonly\s+)?(?:static\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s*:\s*([^;=]+)(?:[;=]|$)/);
            if (propertyMatch) {
...
//# sourceMappingURL=offchain-parser.js.map