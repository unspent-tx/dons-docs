export class AikenParser {
     ...
     */
    static parseImports(content, source = "stdlib") {
        const imports = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const useMatch = line.match(/^use\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\/[a-zA-Z_][a-zA-Z0-9_]*)*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)\s*(?:\.{([^}]+)})?\s*(?:as\s+([a-zA-Z_][a-zA-Z0-9_]*))?\s*$/);
            if (useMatch) {
     ...
     */
    static parseFunctions(content, source = "stdlib") {
        const functions = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            if (fnMatch) {
                ...
                while (testStartLine < lines.length) {
                    ...
                    const testMatch = testLine.match(/^test\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/);
                    if (testMatch) {
     ...
     */
    static parseTypes(content, source = "stdlib") {
        const types = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            const typeMatch = line.match(/^(pub\s+)?(opaque\s+)?type\s+([a-zA-Z_][a-zA-Z0-9_]*(?:<[^>]*>)?)\s*([={].*)?$/);
            if (typeMatch) {
                ...
                if (definitionStart) {
                    if (definitionStart.startsWith("=")) {
                        ...
                        let nextLineIndex = i + 1;
                        let braceCount = (definitionStart.match(/\{/g) || []).length -
                    ...
                    while (nextLineIndex < lines.length) {
                        ...
                        if (!nextLine.startsWith("//") && !nextLine.startsWith("///")) {
                            if (nextLine.startsWith("{")) {
                                // Enum type format
                                fullDefinition = nextLine;
                                let braceCount = (nextLine.match(/\{/g) || []).length -
                                    (nextLine.match(/\}/g) || []).length;
                                nextLineIndex++;
                                // Continue collecting lines until we find the closing brace
                                while (nextLineIndex < lines.length && braceCount > 0) {
     ...
     */
    static parseConstants(content, source = "stdlib") {
        const constants = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            const constMatch = line.match(/^(pub\s+)?const\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*(?::\s*([^=]+?))?\s*=\s*(.+)/);
            if (constMatch) {
    ...
    static parseParameters(paramsStr) {
        ...
        for (const part of paramParts) {
            ...
            const paramMatch = param.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\s*(?::\s*(.+))?$/);
            if (paramMatch) {
     ...
     */
    static isVodkaReExportFile(moduleName, source) {
        if (source !== "vodka")
            return false;
     ...
     */
    static parseVodkaReExports(content, moduleName) {
        ...
        for (const line of lines) {
            ...
            const reExportMatch = trimmed.match(/^pub\s+const\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*)/);
            if (reExportMatch) {
     ...
     */
    static extractDependencies(imports) {
