...
async function generateOffchainData() {
  try {
    console.log("🔍 Generating Offchain library data...");
    ...
    // Filter for offchain packages only
    const offchainPackages = enabledPackages.filter(
      (pkg) => pkg.parsingStrategy === "offchain"
    );

    console.log(
      `📦 Found ${offchainPackages.length} enabled offchain packages:`,
      offchainPackages.map((p) => p.id)
    );

    // Log package details for debugging
    offchainPackages.forEach((pkg) => {
      ...
      console.log(`Package ${pkg.id}:`, {
        ...
        exists,
        enabled: pkg.enabled,
        parsingStrategy: pkg.parsingStrategy,
      });
    ...
    // Initialize SDK with offchain packages only
    const sdk = createAikenSDK(offchainPackages);

    // Load library with all enabled offchain sources
    console.log(
      "Loading offchain library with sources:",
      offchainPackages.map((pkg) => pkg.id)
    );
    await sdk.loadLibrary({
      sources: offchainPackages.map((pkg) => pkg.id),
      includePrivate: true,
    });
    ...
    console.log(`✅ Parsed offchain data:`, {
    ...
    const expectedSources = offchainPackages.map((pkg) => pkg.id);
    ...
    if (missingSources.length > 0) {
      console.warn(
        `⚠️ Missing offchain packages in generated data: ${missingSources.join(
          ", "
        )}`
      );
      ...
      if (actualSources.length === 0) {
        console.error(
          "❌ No offchain packages were successfully loaded. Build may fail."
        );
        console.error(
          "   Please check that all required offchain packages are available."
        );
      } else {
        console.log(
          `✅ Continuing with ${actualSources.length} available offchain packages`
        );
      }
    } else {
      console.log(
        "✅ All expected offchain packages are present in generated data"
      );
    }
    ...
    const outputPath = path.join(publicDir, "offchain-data.json");
    fs.writeFileSync(outputPath, JSON.stringify(data, null, 2));

    console.log(`💾 Generated Offchain data: ${outputPath}`);
    ...
    console.error("❌ Error generating Offchain data:", error);
